...
describe('Screenshot', () => {
    ...
    for(var comptDevice=0;comptDevice<confDevice.length;comptDevice++){
        ...
        for(var compt=0;compt<listMarkets.length;compt++){
            let temp=compt;
            it(''+comptDevice+'/'+confDevice.length+' | Kraken open url | '+confWidth+'x'+confHeight+'-'+listMarkets[temp][0]+'',()=>{
                console.log('=======>'+confWidth+' X '+confHeight);
                cy.checkUtilTakeScreenShotIfNotErrorPage(listMarkets[temp][1],confOnlyStatus200);
            });
            it(''+comptDevice+'/'+confDevice.length+' | Kraken check and close Evidon Banner if present | '+confWidth+'x'+confHeight+'-'+listMarkets[temp][0]+'',()=>{
                cy.checkUtilCloseCookieBanner('.evidon-banner-acceptbutton');
            });
            it(''+comptDevice+'/'+confDevice.length+' | Kraken check and close Non Evidon Banner if present | '+confWidth+'x'+confHeight+'-'+listMarkets[temp][0]+'',()=>{
                cy.checkUtilCloseCookieBanner('#_evh-ric-c');
            });
            it(''+comptDevice+'/'+confDevice.length+' | Kraken take screenshot |'+confWidth+'x'+confHeight+'-'+listMarkets[temp][0]+'',()=>{
